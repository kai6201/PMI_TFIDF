...
var_list$match1 <- str_match_all(var_list$Variable_1,var_list$Variable_2) %>% as.character() %>% as.data.frame()
var_list$match2 <- str_match_all(var_list$Variable_2,var_list$Variable_1) %>% as.character() %>% as.data.frame()
var_list <- var_list %>% filter(match1=="character(0)" & match2=="character(0)")
PPMI_index <- occurrence_data_PPMI[,"index"] %in% var_list[,"index"]
